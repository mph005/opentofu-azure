terraform {
  ...
  backend "azurerm" {
    ...
    # tofu init -backend-config="resource_group_name=tfstate" -backend-config="storage_account_name=tfstate12345" -backend-config="container_name=tfstate" -backend-config="key=dev.tofu.tfstate"
  }
}
